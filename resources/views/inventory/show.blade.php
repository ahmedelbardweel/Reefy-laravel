...
<div class="min-h-screen bg-background-light dark:bg-background-dark pb-24 font-body">
    <!-- Breadcrumbs -->
    <div class="max-w-7xl mx-auto px-4 lg:px-10 py-6">
        <nav class="flex flex-wrap gap-2 text-sm">
            <a class="text-text-secondary-light hover:text-primary transition-colors font-medium" href="{{ route('dashboard') }}">{{ __('market.nav.home') }}</a>
            <span class="text-text-secondary-light font-medium">/</span>
            <a class="text-text-secondary-light hover:text-primary transition-colors font-medium" href="{{ route('inventory.index', ['mode' => 'market']) }}">{{ __('market.title') }}</a>
            <span class="text-text-secondary-light font-medium">/</span>
                    ...
                    <form id="addToCartForm" action="{{ route('cart.add') }}" method="POST">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex items-center border border-border-light dark:border-border-dark rounded-lg bg-white dark:bg-surface-dark h-12 w-32">
                                <button type="button" onclick="decrementQty()" class="w-10 h-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-background-dark text-xl font-bold rounded-r-lg text-text-main dark:text-white">-</button>
                                ...
                                <button type="button" onclick="incrementQty()" class="w-10 h-full flex items-center justify-center hover:bg-gray-100 dark:hover:bg-background-dark text-xl font-bold rounded-l-lg text-text-main dark:text-white">+</button>
                            </div>
                ...
                <div class="bg-white dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-4 shadow-sm flex items-center justify-between gap-4 mt-2">
                    <div class="flex items-center gap-3">
                        <div class="size-14 rounded-full bg-gray-200 bg-cover bg-center border-2 border-white dark:border-border-dark shadow-sm" 
                             style='background-image: url("{{ $product->user->avatar ?? "https://ui-avatars.com/api/?name=".urlencode($product->user->name) }}");'>
                        </div>
                        <div class="flex flex-col">
                            ...
                            <div class="flex items-center gap-3 text-sm text-text-secondary-light dark:text-gray-400">
                                ...
                                <span class="flex items-center gap-1">
                                    ...
                                    {{ $product->user->address ?? 'Qassim, SA' }}
                                </span>
                            </div>
                        </div>
                    </div>
