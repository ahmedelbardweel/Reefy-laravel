...
{
    /**
     * Update negotiation price and quantity
     *
    ...
    public function update(Request $request, Negotiation $negotiation)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'تم تحديث المفاوضة بنجاح',
            'negotiation' => $negotiation->load(['product', 'conversation'])
        ]);
    }

    /**
     * Confirm negotiation and add to cart
     *
     * @param Request $request
     * @param Negotiation $negotiation
     * @return \Illuminate\Http\JsonResponse
    ...
    public function confirm(Request $request, Negotiation $negotiation)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'تم تثبيت الاتفاق وإضافة المنتج لسلة العميل بنجاح',
            'negotiation' => $negotiation->load(['product', 'conversation']),
    ...
    /**
     * Authorize user access to negotiation
     *
     * @param Negotiation $negotiation
     * @return void
     */
    private function authorizeAccess(Negotiation $negotiation)
    {
        ...
        if ($conversation->sender_id !== $user->id && $conversation->receiver_id !== $user->id) {
            abort(403, 'غير مصرح لك بالوصول لهذه المفاوضة');
        }
    }
}
